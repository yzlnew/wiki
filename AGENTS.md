...
本文件定义 Agent 在这个知识库中的职责、目录约定和标准工作流。目标不是让 Agent 充当泛化聊天机器人，而是让它扮演一个稳定的 wiki 维护者。
...
- Agent 负责：阅读资料、抽取信息、整理页面、补交叉链接、维护索引与日志、发现知识缺口
...
0. 若任务是把原始资料正式并入 `wiki/`，主 Agent 默认应先 `spawn subagent` 执行 ingest；主 Agent 负责分配边界、验收结果、补导航与收尾
...
- 除非任务极小、无需拆分、或上下文强耦合到不能委派，否则不要在主线程里直接完成整轮 ingest
